...
# -*- coding: utf-8 -*-
"""
Script Python simple pour intégrer le Lot 1 (modules 4-9) transformés dans dashboard.html
Version sans émojis pour éviter problèmes d'encodage
"""
import re
import sys
import os
...
def main():
    ...
    if '4' in modules:
        ...
        if old_section:
            ...
            print("Section KPIs non trouvee")
    ...
    if '6' in modules:
        ...
        if old_section:
            ...
            print("Section Formateurs non trouvee")
    ...
    if '7' in modules:
        ...
        if old_section:
            ...
            print("Section Graphique revenus non trouvee")
    ...
    if '8' in modules:
        ...
        if old_section:
            ...
            print("Section Étudiants à risque non trouvee")
    
    # 5. REMPLACEMENT Progression des Cohortes (Module 9)
    # Section Cohortes - plus complexe car dans template string
    cohorte_start = '        <!-- ── Progression des Cohortes ───────────────────────── -->'
    # Chercher jusqu'à la fin de la template string (`)
    if '9' in modules:
        # Pattern: depuis cohorte_start jusqu'à "`) // Load async sections after DOM"
        pattern = re.escape(cohorte_start) + r'.*?`\)\s*// Load async sections after DOM'
        old_section = re.search(pattern, dashboard_content, re.DOTALL)
        if old_section:
            # On remplace tout sauf la fin
            new_section = f"{cohorte_start}\n{modules['9']}\n"
            # Garder la fin "`) // Load async sections after DOM"
            ...
            print("Section Progression des Cohortes non trouvee")
    ...
    if '5' in modules:
        ...
        if old_section:
            ...
            print("Section Agenda du jour non trouvee")
    
    # Écriture du fichier de sortie
    print(f"\nEcriture du fichier {output_path}...")
    write_file_utf8(output_path, dashboard_content)
    
    # Statistiques
    print("\n" + "="*50)
    print("RECAPITULATIF DE L'INTEGRATION LOT 1")
    print("="*50)
    for mod in required_modules:
        status = "INTEGRE" if mod in modules else "MANQUANT"
        print(f"Module {mod}: {status}")
    
    print("\nIntegration Lot 1 terminee !")
    print(f"Fichier genere : {output_path}")
    print("Verifiez que les IDs, data-attributes et onclick sont preserves.")
